"""OpenAI-compatible shim that forwards Hermes requests to ``codex-mistral``.

Hermes expects an OpenAI-style ``client.chat.completions.create(...)`` surface.
This adapter spawns the local ``codex-mistral`` CLI, lets Codex perform the
...
CODEX_MISTRAL_MARKER_BASE_URL = "codex+mistral://local"
...
def _extract_assistant_text(stdout: str) -> str:
    ...
    return "\n".join(part for part in assistant_parts if part).strip() or fallback


class _CodexMistralStream:
    def __init__(self, text: str):
        self._chunks = _chunk_text(text)
        self._index = 0

    def __iter__(self) -> "_CodexMistralStream":
        return self
...
class _CodexMistralChatCompletions:
    def __init__(self, client: "CodexMistralCLIClient"):
        self._client = client
...
class _CodexMistralChatNamespace:
    def __init__(self, client: "CodexMistralCLIClient"):
        self.completions = _CodexMistralChatCompletions(client)


class CodexMistralCLIClient:
    """Minimal OpenAI-style client facade backed by ``codex-mistral``."""

    def __init__(
        self,
        *,
        api_key: str,
        base_url: str = CODEX_MISTRAL_MARKER_BASE_URL,
        command: str | None = None,
        args: list[str] | None = None,
    ) -> None:
        self.api_key = api_key
        self.base_url = base_url or CODEX_MISTRAL_MARKER_BASE_URL
        self.command = command or "codex-mistral"
        self.args = list(args or [])
        self.chat = _CodexMistralChatNamespace(self)
    ...
    def _run_codex(self, *, model: str | None, prompt: str, timeout: float) -> str:
        env = os.environ.copy()
        env["MISTRAL_API_KEY"] = self.api_key
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(f"codex-mistral failed: {detail}")
        return _extract_assistant_text((result.stdout or "").strip())
    ...
    def _create_chat_completion(self, **kwargs: Any) -> Any:
        ...
        if stream:
            return _CodexMistralStream(text)

        message = SimpleNamespace(role="assistant", content=text, tool_calls=None)
        choice = SimpleNamespace(index=0, message=message, finish_reason="stop")
        return SimpleNamespace(
            id=f"codex-mistral-{int(time.time() * 1000)}",
            object="chat.completion",
            created=int(time.time()),
            model=model or "codex-mistral",
            choices=[choice],
            usage=None,
        )
