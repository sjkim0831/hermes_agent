...
# Configure the current Hermes checkout to use the codex-cerebras backend and
# register multiple selectable Cerebras API accounts without storing secrets in git.
...
ACCOUNT_COUNT="${CEREBRAS_ACCOUNT_COUNT:-18}"
MODEL="${CEREBRAS_MODEL:-qwen-3-235b-a22b-instruct-2507}"
...
if [[ ! -f "$ENV_PATH" ]]; then
  ...
  echo "Created $ENV_PATH from template. Fill in your CEREBRAS_API_KEY_101... values."
else
  for i in $(seq 101 $((100 + ACCOUNT_COUNT))); do
...
export CONFIG_PATH ACCOUNT_COUNT MODEL GEMINI_ACCOUNT_COUNT GEMINI_MODEL
python3 - <<'PY'
import os
...
account_count = int(os.environ["ACCOUNT_COUNT"])
...
if not isinstance(model, dict):
    model = {"default": str(model or model_name)}
config["model"] = model
...
providers = []
for i in range(101, 101 + account_count):
    providers.append({
...
kept = [
    ...
    if not (
        ...
        and (
            ...
            or (
                str(entry.get("provider_key", "")).strip() == "gemini"
                and str(entry.get("name", "")).strip().startswith("Gemini API ")
            )
        )
    )
]
config["custom_providers"] = providers + gemini_providers + kept
...
echo "Hermes codex-cerebras config installed:"
